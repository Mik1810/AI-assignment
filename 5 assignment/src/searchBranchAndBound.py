...
from .searchProblem import acyclic_delivery_problem
class DF_branch_and_bound(Searcher):
    """returns a branch and bound searcher for a problem.    
    An optimal path with cost less than bound can be found by calling constraint()
    """
    def __init__(self, problem, bound=float("inf")):
        """creates a searcher than can be used with constraint() to find an optimal path.
